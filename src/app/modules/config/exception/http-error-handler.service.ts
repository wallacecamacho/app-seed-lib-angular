...
export class HttpErrorHandler {
  ...
  createHandleError = (serviceName = '') => <T>(
    ...
    operation = 'operation',
    result = {} as T
  ) => this.handleError(serviceName, application, operation, result)
   ...
   * @param urlEndpointMessage - url to save the logs
    ...
    return (error: HttpErrorResponse): Observable<T> => {
      ...
      const message =
        error.error instanceof ErrorEvent
          ? error.error.message
          : `O servidor retornou o codigo ${error.status} com o body '${
              error.statusText
            }'`;

      // TODO: better job of transforming error for user consumption
      this.messageService.add(
        `${serviceName}: ${operation} falhou: ${message}`
      );
